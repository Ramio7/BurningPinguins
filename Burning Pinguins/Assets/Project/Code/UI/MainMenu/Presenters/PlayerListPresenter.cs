...
public class PlayerListPresenter : MonoBehaviourPunCallbacks, IUiList
{
    ...
    {
        foreach (var player in PhotonNetwork.CurrentRoom.Players) _players.Add(new(player.Value, _playersUiContainer, _playerInfoContainerPrefab));
    }

    public override void OnLeftRoom()
    {
        foreach (var player in _players) Destroy(player.gameObject);
        _players.Clear();
    }
}
